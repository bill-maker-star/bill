...
function closeMainMenu() {
    const dropdown = document.getElementById('main-menu-dropdown');
    const button = document.querySelector('.menu-dots-btn');
...
document.addEventListener('click', function(event) {
    if (!event.target.closest('.main-menu-more')) {
        closeMainMenu();
    }
});

function getSavedPaymentSettings() {
    try {
        return JSON.parse(localStorage.getItem('restaurantPaymentSettings')) || {};
    } catch (error) {
        return {};
    }
}
...
function initializePaymentSettings() {
    const settings = getSavedPaymentSettings();
    const menuPasswordInput = document.getElementById('menu-encryption-password');
    const qrPasswordInput = document.getElementById('password');
    const razorpayKeyInput = document.getElementById('razorpay-key-id');
    const razorpaySecretInput = document.getElementById('razorpay-key-secret');

    if (menuPasswordInput) menuPasswordInput.value = settings.encryptionPassword || '';
    if (qrPasswordInput && settings.encryptionPassword) qrPasswordInput.value = settings.encryptionPassword;
    if (razorpayKeyInput) razorpayKeyInput.value = settings.razorpayKeyId || '';
    if (razorpaySecretInput) razorpaySecretInput.value = settings.razorpaySecret || '';

    if (menuPasswordInput && qrPasswordInput) {
        menuPasswordInput.addEventListener('input', function() {
            qrPasswordInput.value = menuPasswordInput.value;
        });
        qrPasswordInput.addEventListener('input', function() {
            menuPasswordInput.value = qrPasswordInput.value;
        });
    }
}
...
function savePaymentSettings(event) {
    ...
    const settings = {
        encryptionPassword: document.getElementById('menu-encryption-password')?.value.trim() || '',
        razorpayKeyId: document.getElementById('razorpay-key-id')?.value.trim() || '',
        razorpaySecret: document.getElementById('razorpay-key-secret')?.value.trim() || ''
    };

    localStorage.setItem('restaurantPaymentSettings', JSON.stringify(settings));

    const qrPasswordInput = document.getElementById('password');
    if (qrPasswordInput) qrPasswordInput.value = settings.encryptionPassword;
...
function payWithRazorpay() {
    ...
    if (!settings.razorpayKeyId) {
        alert('Please add your Razorpay Key ID from the three-dot menu first.');
        return false;
    }

    if (!updateCurrentBillFromForm()) {
        return false;
    }

    if (typeof Razorpay === 'undefined') {
        alert('Razorpay checkout script is not loaded. Please check your internet connection.');
        return false;
    }
    ...
    const options = {
        ...
        handler: function(response) {
            document.getElementById('payment-method').value = 'Online Pay';
            currentBill.paymentMethod = 'Razorpay';
            currentBill.razorpayPaymentId = response.razorpay_payment_id;
            generateReceipt();
        ...
        modal: {
            ondismiss: function() {
                alert('Razorpay payment window closed.');
            }
        }
    };
...
function updateCurrentBillFromForm(showEmptyAlert = true) {
    currentBill.items = [];
    ...
    if (currentBill.items.length === 0) {
        if (showEmptyAlert) alert('Please add at least one item to the bill.');
        return false;
    }
...
async function generateQR() {
    ...
    if (currentBill.paymentMethod === 'Online Pay') {
        payWithRazorpay();
        return;
    }
...
function initializeQRGenerator() {
    ...
    window.autoFillQRGeneratorFromBill = autoFillQRGeneratorFromBill;
}
